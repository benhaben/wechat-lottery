  ...
  32: function(t, e, n) {
    ...
    async function _(t, e) {
      ...
      try {
        ...
        if (t && t.total_prize === _ * a.c.MONEY_UNIT) {
          console.log(
            `支付金额和抽奖一致 total_prize: ${t.total_prize}, totalCost: ${_}`
          );
          let n = r.e.getWithoutData(u);
          n.set("status", a.c.WAIT_APPROVE),
            n.set("transaction_no", o),
            e(null, await n.update());
        } else {
          console.log(
            `支付金额和抽奖不一致 total_prize: ${t.total_prize}, totalCost: ${_}`
          );
          let n = {
            error: `支付金额和抽奖金额不一致：want - ${t.total_prize /
              a.c.MONEY_UNIT} | actual - ${_}`,
