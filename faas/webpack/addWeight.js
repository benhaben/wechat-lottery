exports.main = (function(e) {
  ...
  function(e, t, n) {
    ...
    async function _(e, t) {
      const { lottery_id: n, weight: _, attend_id: a } = e.data,
        o = e.request.user.id;
      try {
        let e = (await r.j.get(o)).data;
        console.log(`cost : ${_}，user : ${JSON.stringify(e)}`);
        let i = `增加权重减少${_}运气值`;
        if (e.lucky_num < _) throw new Error(u.d.OUT_OF_LUCKY_NUM);
        let s = r.j.getWithoutData(o);
        s.incrementBy("lucky_num", -_);
        let c = await s.update();
        console.log(`userUpdateRet : ${JSON.stringify(c)}`);
        const E = r.b.create();
        let T = await E.set({
          reason: i,
          lucky_num: -_,
        ...
        console.log(`createBalanceRecordRet : ${JSON.stringify(T)}`);
        const l = r.i.getWithoutData(a);
        l.incrementBy("weight", _);
        let d = await l.update();
        console.log(`updateUserLotteryRet : ${JSON.stringify(d)}`), t(null, !0);
      } catch (e) {
        t(e);
      }
    }
