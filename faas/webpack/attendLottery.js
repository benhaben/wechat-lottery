exports.main = (function(t) {
  ...
  function(t, e, n) {
    ...
    const u = new BaaS.TableObject(r.d.USER_LOTTERY_RECORD),
      T = new BaaS.TableObject(r.d.LOTTERY),
      _ = new BaaS.TableObject(r.d.CONFIG),
      a = new BaaS.TableObject(r.d.ERROR),
      E = new BaaS.User(),
      o = new BaaS.TableObject(r.d.BALANCE_LUCKY_RECORD),
      c = new BaaS.TableObject(r.d.WITHDRAW);
    async function O(t) {
      let e = new BaaS.Query();
      return (
        e.compare("lottery", "=", T.getWithoutData(t)), u.setQuery(e).count()
      );
    }
    async function i() {
  ...
  function(t, e, n) {
    ...
    async function T(t, e) {
      const { lottery_id: n, weight: T } = t.data,
        _ = t.request.user.id;
      try {
        let t,
          a = T / u.b.ONE_LUCKY_NUM_WEIGHT,
          E = await r.i.get(_);
        if (
          (0 === a
            ? ((t = `参与抽奖，减少${u.b.ATTEND_LOTTERY_COST}运气值`),
              (a = u.b.ATTEND_LOTTERY_COST))
            : ((t = `参与抽奖，减少${u.b.ATTEND_LOTTERY_COST}运气值，增加权重减少${a}运气值`),
              (a = u.b.ATTEND_LOTTERY_COST + a)),
          E.lucky_num < a)
        )
          throw new Error(u.c.OUT_OF_LUCKY_NUM);
        let o = r.i.getWithoutData(_);
        o.incrementBy("lucky_num", -a), await o.update();
        const c = r.b.create();
        await c
          .set({ reason: t, lucky_num: -a, user_id: _, lottery_id: n })
          .save();
