exports.main = (function(e) {
  ...
  1: function(e, t, n) {
    ...
    async function R(e) {
      ...
      return (
        t.compare("lottery", "=", s.getWithoutData(e)), c.setQuery(t).count()
      );
    }
    async function d() {
  ...
  16: function(e, t, n) {
    ...
    async function _(e, t) {
      ...
      try {
        let e,
          o = _,
          c = (await r.j.get(a)).data;
        if (
          (console.log(`user : ${JSON.stringify(c)}`),
          0 === o
            ? ((e = `参与抽奖，减少${u.c.ATTEND_LOTTERY_COST}运气值`),
              (o = u.c.ATTEND_LOTTERY_COST))
            : ((e = `参与抽奖，减少${u.c.ATTEND_LOTTERY_COST}运气值，增加权重减少${o}运气值`),
              (o = u.c.ATTEND_LOTTERY_COST + o)),
          console.log(`cost : ${o}`),
          c.lucky_num < o)
        )
          throw new Error(u.d.OUT_OF_LUCKY_NUM);
        let s = r.j.getWithoutData(a);
        s.incrementBy("lucky_num", -o);
        let i = await s.update();
        console.log(`userUpdateRet : ${JSON.stringify(i)}`);
        const T = r.b.create();
        let E = await T.set({
          reason: e,
          lucky_num: -o,
        ...
        console.log(`createBalanceRecordRet : ${JSON.stringify(E)}`);
        let l = r.e.getWithoutData(n);
        const O = r.i.create();
        let R = await O.set({
          user_id: a,
          user: s,
          nickname: c.nickname,
          avatar_cache: c.avatar,
          lottery: l,
          weight: _
        }).save();
        console.log(`createUserLotteryRet : ${JSON.stringify(R)}`), t(null, !0);
      } catch (e) {
        t(e);
      }
    }
