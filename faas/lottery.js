...
const updateLotteryRecord = (offset, lotteryID) => {
  ...
  let luckyRecord = userLotteryRecordTable
    .limit(1)
    .offset(offset)
    .getWithoutData(luckyQuery);
...
const draw = numberOfPeople => times => {
  ...
  for (let i = 0; i < times; i++) {
    ...
    */
    res.forEach(r => {
      if (r <= luckyOffset) luckyOffset += 1;
    });
...
function mydraw(numberOfPeople, times) {
  let res = [];
  for (let i = 0; i < times; i++) {
...
exports.main = async function lottery(event, callback) {
  let lotteryData = await lotteryTable
    .get(LOTTERY_RECORD)
  ...
  let userLotteryRecords = await userLotteryRecordTable
    .setQuery(query)
    .limit(1)
    .find()
    .catch(err => callback(err));
  ...
  const templateMessageResult = await BaaS.sendTemplateMessage(data).catch(
    err => callback(err)
  );
